...
def test_flash_attention_patch(dtype=torch.float16, device="cuda:0"):
    ...
    device2 = "cuda:1"
    patched_model = LlamaForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf", torch_dtype=dtype).to(device2)
    ...
    with torch.no_grad():
        for layer1, layer2 in zip(model.model.layers, patched_model.model.layers):
            ...
            out2, _, _ = attn2(hidden_states.to(device2), attention_mask=attn_mask.to(device2), position_ids=position_ids.to(device2))

            assert (((out1 - out2.to(device)) * attention_mask.unsqueeze(-1)).mean(dim=-1).abs() < 1e-3).all()
