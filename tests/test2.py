...
def test_flash_attention_patch(dtype=torch.float16, device="cuda:0"):
    ...
    store_dicts[2] = {}
    with torch.no_grad():
        for i,layer1 in enumerate(model.model.layers):
            ...
            attn_mask = model.model._prepare_decoder_attention_mask(attention_mask, (4, 10), hidden_states, 0)
            position_ids = torch.arange(10, device=device).unsqueeze(0).expand(4, -1)
            attn1, attn2 = layer1.self_attn
