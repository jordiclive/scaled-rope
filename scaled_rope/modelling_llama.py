...
def find_correction_range(
    ...
    low = math.floor(
        find_correction_factor(low_rot, dim, base, max_position_embeddings)
    )
    high = math.ceil(
        find_correction_factor(high_rot, dim, base, max_position_embeddings)
    )
...
class LlamaPartNTKScaledRotaryEmbedding(torch.nn.Module):
    def __init__(
        ...
        inv_freq_ntk = 1.0 / (
            find_newbase_ntk(dim, base, scale)
        ...
        inv_freq_mask = (
            1
            - linear_ramp_mask(low, high, dim // 2)
            .type(current_dtype)
            .to(current_device)
        ) * ntk_factor
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if config.use_xpos:
            ...
            self.rotary_emb = LlamaPartNTKScaledRotaryEmbedding(
                self.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                scale=config.part_ntk_scale,
            )
        else:
            self.rotary_emb = LlamaScaledRotaryEmbedding(
                self.head_dim,
                max_position_embeddings=self.max_position_embeddings,
