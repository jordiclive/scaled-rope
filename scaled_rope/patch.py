...
def patch_llama_for_dynamic_scaled_rotary_embeddings(model, ntk):
    from .LlamaDynamicScaledRotaryEmbedding import \
        LlamaDynamicScaledRotaryEmbedding
...
def patch_llama_for_linear_scaled_rotary_embeddings(model, scale):
    from .LlamaLinearScaledRotaryEmbedding import \
        LlamaLinearScaledRotaryEmbedding
...
def patch_gptneox_for_ntk_scaled_rotary_embeddings(model, alpha):
    from .GPTNeoXNTKScaledRotaryEmbedding import \
        GPTNeoXNTKScaledRotaryEmbedding
    ...
    for each in model.gpt_neox.layers:
        each.attention.bias = torch.tril(
            torch.ones(
                ...
                dtype=each.attention.bias.dtype,
                device=each.attention.bias.device,
            )
